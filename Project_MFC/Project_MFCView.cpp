...
void CProjectMFCView::OnDraw(CDC* pDC)
{
	...
	for (int ipoint = 0; ipoint < npoints; ++ipoint)
	{
		...
		COLORREF my_color = (*pDoc->pDat)[ipoint].color;
		...
		//Output text
		pDC->TextOut(scr.x + PointRad + 2, scr.y, (*pDoc->pDat)[ipoint].name);
	}
...
void CProjectMFCView::OnOperateGraphwind()
{
	...
	dlg.lf = lf;
	dlg.PointRad = PointRad;
	if (dlg.DoModal() == IDOK)
	{
		PointRad = dlg.PointRad;
		lf = dlg.lf;
		ASSERT_VALID(pDoc);
		pDoc->SetModifiedFlag();
		pDoc->UpdateAllViews(NULL);
		pDoc->pExcept->PutMessage(1003);
	}
}
